...
BOOL CChildFrame::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext* pContext)
{
	...
	if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CLeftView), CSize(100, 100), pContext) ||
		!m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CSeraphMFCView), CSize(100, 100), pContext))
	{
...
CSeraphMFCView* CChildFrame::GetRightPane()
{
	CWnd* pWnd = m_wndSplitter.GetPane(0, 1);
	CSeraphMFCView* pView = DYNAMIC_DOWNCAST(CSeraphMFCView, pWnd);
	return pView;
}
...
void CChildFrame::OnUpdateViewStyles(CCmdUI* pCmdUI)
{
	...
	CSeraphMFCView* pView = GetRightPane(); 
...
void CChildFrame::OnViewStyle(UINT nCommandID)
{
	...
	// if the right-hand pane has been created and is a CSeraphMFCView, process the menu commands...
	if (pView != NULL)
	{
		int nStyle = -1;
