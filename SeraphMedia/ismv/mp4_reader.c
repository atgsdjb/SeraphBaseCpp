                                  ...
                                  traf_t const* traf)
{
  ...
  if (trun->flags_ & 0x000001)
	prev_run_end = traf->tfhd_->base_data_offset_ + trun->data_offset_;
  else
    prev_run_end = traf->tfhd_->base_data_offset_;
...
//    int64_t pts = trak->fragment_pts_;  // trak->mdia_->mdhd_->duration_;
	...
	if (trun->flags_ & 0x000001)
		data_offset = tfhd->base_data_offset_ + trun->data_offset_;
	else
		data_offset = prev_run_end;
    //
    if(pts == 0 && trak->edts_)
    {
...
//      trak->fragment_pts_ += trun_table->sample_duration_;
	...
	{
		uint64_t*p = (uint64_t*)&mp4_context->moof_offset_;
		prev_run_end = data_offset;
		*p= prev_run_end;
	}
  }
...
static int trak_build_index(mp4_context_t const* mp4_context, trak_t* trak)
{
  ...
  if(have_samples)
  {
    ...
    if(stss)
    {
      ...
      // can be very small.
	  trak->smoothes_size_ = stss->entries_;
	  trak->smoothes_ = (struct smooth_t*)malloc((trak->smoothes_size_ + 1) * sizeof(struct smooth_t));
      for(i = 0; i != stss->entries_; ++i)
      {
        uint32_t s = stss->sample_numbers_[i] - 1;
        trak->samples_[s].is_ss_ = 1;
        trak->samples_[s].is_smooth_ss_ = 1;
		trak->smoothes_[i].start = s;
      }
	  trak->smoothes_[trak->smoothes_size_].start = trak->samples_size_;
    }
    else
    {
      for(i = 0; i != trak->samples_size_; ++i)
...
static void copy_sync_samples_to_audio_track(trak_t* video, trak_t* audio)
{
  if(video)
  {
	...
	audio->smoothes_size_ = 0;
	audio->smoothes_ = (struct smooth_t*)malloc((video->smoothes_size_ + 1) * sizeof(struct smooth_t));
    while(first != last)
    {
      if(first->is_smooth_ss_)
      {
        ...
        while(audio_first != audio_last)
        {
          if(audio_first->pts_ >= pts)
          {
			...
			audio->smoothes_[audio->smoothes_size_++].start = audio_first - audio->samples_;
            break;
          }
          ++audio_first;
        }
		audio->smoothes_[audio->smoothes_size_].start = audio->samples_size_;
      }
      ++first;
    }
  }
  else
  {
	...
	while(f != l)
	{
		if(f->is_smooth_ss_)
		{
			audio->smoothes_[increment++].start = f - audio->samples_;
		}
		++f;
	}
	audio->smoothes_[increment].start = audio->samples_size_;
  }
}
