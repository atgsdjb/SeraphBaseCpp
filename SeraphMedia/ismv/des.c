...
int ZN_E3des(char* iostr,int len, unsigned char key[24])
{
	int i;
	des3_context ctx3;
	...
	if (len % 8 != 0)
		return -1;
	// Data = Data + CString(' ', 8 - Data.GetLength() % 8);
	for (i=0;i<len;i+=8)
		des3_encrypt(&ctx3, iostr+i, iostr+i);
	return len;
}

int ZN_D3des(char* iostr,int len, unsigned char key[24])
{
	int i;
	des3_context ctx3;
	des3_set_3keys(&ctx3, key, key+8, key+16);
	...
	// Data = Data + CString(' ', 8 - Data.GetLength() % 8);
	for (i=0;i<len;i+=8)
		des3_decrypt(&ctx3, iostr+i, iostr+i);
	return len;
}
