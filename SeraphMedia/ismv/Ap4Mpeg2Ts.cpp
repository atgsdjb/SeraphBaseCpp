...
//checksum
AP4_UI16 InCksum(AP4_UI08 *pData, int len)
{
...
private:
    AP4_Mpeg2TsAudioSampleStream(AP4_UI16 pid, AP4_UI08 type, AP4_UI32 timescale) : 
...
AP4_Mpeg2TsAudioSampleStream::Create(AP4_UI16                          pid,
                                     ...
                                     AP4_Mpeg2TsWriter::SampleStream*& stream)
{
    stream = new AP4_Mpeg2TsAudioSampleStream(pid, type, timescale);
    return AP4_SUCCESS;
}
                                          ...
                                          AP4_ByteStream&        output)
{
	...
	if (audio_desc->GetMpeg4AudioObjectType() != AP4_MpegAudioSampleDescription::AP4_MPEG4_AUDIO_OBJECT_TYPE_AAC_LC) return AP4_ERROR_NOT_SUPPORTED;
    ...
    if (AP4_SUCCEEDED(sample.ReadData(data))) {
		// check sum
		AP4_UI16 cksum = 0;
		cksum = InCksum(data.UseData(), data.GetDataSize());
		if(cksum != sample.GetCkSum())
		{
			//fprintf(stderr, "error cksum in audio data\n");
		}
		
		if (AP4_MPEG2_STREAM_TYPE_AAC == audio_desc->GetType())
        {
			...
			AP4_UI64 ts = sample.GetDts();
			WritePES(buffer, 7+sample.GetSize(), ts, false, ts, with_pcr, output);
			delete[] buffer;
        }
		else if (AP4_MPEG2_STREAM_TYPE_MP3 == audio_desc->GetType())
		{
			unsigned char* buffer = (unsigned char*)data.GetData();
			AP4_UI64 ts = AP4_ConvertTime(sample.GetDts(), m_TimeScale, 90000);
			WritePES(buffer, sample.GetSize(), ts, false, ts, with_pcr, output);
        }
...
private:
    AP4_Mpeg2TsVideoSampleStream(AP4_UI16 pid, AP4_UI08 type, AP4_UI32 timescale) :
        AP4_Mpeg2TsWriter::SampleStream(pid, 
                                        AP4_MPEG2_TS_DEFAULT_STREAM_ID_VIDEO, 
                                        type,
                                        timescale),
    m_bSpsAndPpsDone(false),
    m_NaluLengthSize(0) {}
    
    bool           m_bSpsAndPpsDone;
                                     ...
                                     AP4_Mpeg2TsWriter::SampleStream*& stream)
{
    ...
    stream = new AP4_Mpeg2TsVideoSampleStream(pid, type, timescale);        
    return AP4_SUCCESS;
}
...
+---------------------------------------------------------------------*/
bool 
AP4_GetStartCode(const unsigned char*& pBegin, const unsigned char*& pStart, int& cRemain)
{
	...
	const unsigned char* pThis = pStart;
	int cBytes = cRemain;
                                          ...
                                          AP4_ByteStream&        output)
{
    ...
    if ( !m_bSpsAndPpsDone ) {
        ...
        m_bSpsAndPpsDone = true;

        // make the SPS/PPS prefix
		m_NaluLengthSize = avc_desc->GetNaluLengthSize();
        m_Prefix.SetDataSize(0);
        if(avc_desc->GetSequenceParameters().GetDataSize() > 0){
            AP4_DataBuffer& buffer = avc_desc->GetSequenceParameters();
        ...
        if(avc_desc->GetPictureParameters().GetDataSize() > 0){
            AP4_DataBuffer& buffer = avc_desc->GetPictureParameters();
            unsigned int prefix_size = m_Prefix.GetDataSize();
            m_Prefix.SetDataSize(prefix_size+4+buffer.GetDataSize());
            unsigned char* p = m_Prefix.UseData()+prefix_size;
            *p++ = 0;
	...
	// check sum
	AP4_UI16 cksum = 0;
	cksum = InCksum(sample_data.UseData(), sample_data.GetDataSize());
	if(cksum != sample.GetCkSum())
	{
		//fprintf(stderr, "error cksum in video data\n");
	}
    ...
    while (data_size) {
        ...
        if (m_NaluLengthSize == 1) {
			...
			const unsigned char* m_pStart = NULL;
			const unsigned char* m_pStartCodeStart = NULL;
			const unsigned char* pBegin = NULL;
			const unsigned char* pBuffer = data;
			int cSpace = data_size;
			if (AP4_GetStartCode(pBegin, pBuffer, cSpace))
			{
				m_pStart = pBuffer;
		...
		eNALType nalu_type = eNALType(data[0] & 0x1F);
		if(NAL_Sequence_Params == nalu_type)
		{
			m_bSpsAndPpsDone = false;
			AP4_AvcSampleDescription* avc_desc = AP4_DYNAMIC_CAST(AP4_AvcSampleDescription, sample_description);
			if(avc_desc) {
				AP4_DataBuffer sps(data, nalu_size);
				avc_desc->SetSequenceParameters(sps);
			}
		}else if (NAL_Picture_Params == nalu_type)
		{
			m_bSpsAndPpsDone = false;
			AP4_AvcSampleDescription* avc_desc = AP4_DYNAMIC_CAST(AP4_AvcSampleDescription, sample_description);
			if(avc_desc){
				AP4_DataBuffer pps(data, nalu_size);
				avc_desc->SetPictureParameters(pps);
			}
		}
	...
	AP4_UI64 dts = sample.GetDts();
	AP4_UI64 pts = sample.GetCts();
...
AP4_Mpeg2TsWriter::WritePAT(AP4_ByteStream& output)
{
    ...
    writer.Write(0, 8);  // pointer 1
    writer.Write(0, 8);  // table_id 2
    writer.Write(1, 1);  // section_syntax_indicator 3
    writer.Write(0, 1);  // '0' 3
    writer.Write(3, 2);  // reserved 3
    writer.Write(13, 12);// section_length 4
    writer.Write(1, 16); // transport_stream_id 5 6
    writer.Write(3, 2);  // reserved 7
    writer.Write(0, 5);  // version_number 7
    writer.Write(1, 1);  // current_next_indicator 7
    writer.Write(0, 8);  // section_number 8
    writer.Write(0, 8);  // last_section_number 9
    writer.Write(1, 16); // program number 10 11
    writer.Write(7, 3);  // reserved 12
    writer.Write(m_PMT->GetPID(), 13); // program_map_PID 12 13
...
+---------------------------------------------------------------------*/
AP4_Result 
AP4_Mpeg2TsWriter::SetAudioStream(AP4_UI08 type, AP4_UI32 timescale, SampleStream*& stream)
{
    // default
    stream = NULL;
...
+---------------------------------------------------------------------*/
AP4_Result 
AP4_Mpeg2TsWriter::SetVideoStream(AP4_UI08 type,  AP4_UI32 timescale, SampleStream*& stream)
{
    // default
    stream = NULL;
