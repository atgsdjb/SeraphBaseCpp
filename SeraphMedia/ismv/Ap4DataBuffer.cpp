...
AP4_DataBuffer::AP4_DataBuffer(AP4_Size buffer_size) :
    ...
    m_DataSize(0)
{
	//UUSee_Printf("begin new AP4_DataBuffer size:%u",buffer_size);
    m_Buffer = new AP4_Byte[buffer_size];
	//UUSee_Printf("end new AP4_DataBuffer size:%u m_Buffer:%d",buffer_size,m_Buffer);
}
...
{
    if (data && data_size) {
		//UUSee_Printf("new AP4_DataBuffer 1 size:%u data:%d",data_size,data);
        m_Buffer = new AP4_Byte[data_size];
		//UUSee_Printf("new AP4_DataBuffer 2 m_Buffer:%d",m_Buffer);
        AP4_CopyMemory(m_Buffer, data, data_size);
		//UUSee_Printf("new AP4_DataBuffer 3 size:%u",data_size);
    }
}
